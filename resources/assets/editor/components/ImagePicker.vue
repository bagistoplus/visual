...
<template>
  <div>
    <div class="flex flex-col gap-2">
      ...
      <div v-if="!model" class="min-h-24 border border-dashed rounded flex flex-col items-center justify-center">
        <button
          @click="openDialog" class="text-blue-600 text-sm bg-zinc-200 rounded px-2.5 py-1.5 hover:bg-zinc-100 hover:text-blue-800">Select image</button>
      </div>
      <div v-else>
        <div class="rounded-t p-3 bg-neutral-100 relative">
          <img :src="model.url" alt="">
          <button class="absolute top-4 right-4 bg-gray-700/30 text-gray-200 p-px rounded" @click="removeImage">
            <XMarkIcon class="w-4 h-4"/>
          </button>
        </div>
        <div class="rounded-b p-3 bg-neutral-100 border-t border-white">
          <button @click="openDialog" class="text-sm w-full text-center bg-white border rounded py-1">Change image</button>
        </div>
      </div>
    </div>
        ...
        <Dialog.Positioner class="flex fixed top-14 left-14 bottom-0 w-[17.85rem] items-center justify-center">
          <Dialog.Content class="bg-white shadow flex flex-col w-full h-full overflow-hidden">
            ...
            <section class="flex-1 flex flex-col gap-3 min-h-0 p-3 overflow-y-auto">
              <div v-bind="fileUpload.getRootProps()" accept="image/*">
                <div v-bind="fileUpload.getDropzoneProps()" class="flex flex-col gap-3 items-center justify-center h-32 bg-zinc-50/50 border border-zinc-300 border-dashed rounded-lg">
                  <p>Drop your images here</p>
                  <button v-bind="fileUpload.getTriggerProps()" class="cursor-pointer bg-blue-500 text-white shadow-lg rounded border px-2.5 py-1.5 text-sm">
                    Add images
                  </button>
                </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <ImagePreview v-for="image in uploadingImages" :key="image.path" :image="image" />
                <ImagePreview
            ...
            <footer class="flex-none flex items-center gap-3 p-3 justify-end h-12 border-t border-neutral-200">
              <button @click="onCancel" class="text-sm shadow px-3 py-1 rounded bg-neutral-100 border">Cancel</button>
