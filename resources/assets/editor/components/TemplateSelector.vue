  ...
  <Menu.Root @select="onSelect">
    ...
    <Menu.Positioner class="w-56">
      <Menu.Content class="pointer-events-none border shadow flex gap-1 p-1 flex-col outline-none rounded bg-white data-[state=open]:animate-fade-in">
        <Menu.Item v-for="t in templates" :key="t.template" :value="t.template" class="rounded cursor-pointer flex items-center h-9 px-3 gap-3 hover:bg-gray-200">
          <HomeIcon class="inline w-4" />
          {{ t.label }}
        </Menu.Item>
      </Menu.Content>
