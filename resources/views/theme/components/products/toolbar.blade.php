...
<div {{ $attributes->merge(['class' => 'mb-6 hidden flex-wrap justify-between gap-4 md:flex']) }}>
  <div class="flex items-center gap-4">
    @if ($availableSortOptions->isNotEmpty())
      <select class="w-auto" name="sort" aria-label="Sort by" wire:model.live="sort">
    ...
    @if ($availablePaginationLimits->isNotEmpty())
      <select class="w-auto" name="limit" aria-label="Items per page" wire:model.live="limit">
  ...
  <div class="flex gap-2">
    <button aria-label="Show as grid" x-on:click="displayMode = 'grid'" class="rounded-lg px-3 py-2"
    ...
    <button aria-label="Show as list" x-on:click="displayMode ='list'" class="rounded-lg px-3 py-2"
